...
for model in models:
  ...
  for i in range(total_jobs):
      ...
      subfile = rundir+"/cond_"+model+"_batch_"+str(i)+".cmd"
      f = open(subfile,"w")
      ...
      f.write("Log="+rundir+"/"+fname+".log.$(Cluster).$(Process)\n")
      f.write("output="+rundir+"/"+fname+".out.$(Cluster).$(Process)\n")
      f.write("error="+rundir+"/"+fname+".err.$(Cluster).$(Process)\n")
      f.write("queue 1\n")
      f.close()
